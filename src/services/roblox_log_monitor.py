...
@sealed
class RobloxLogMonitor(LoggerMixin, IDisposable):
    def __init__(self):
    ...
    def start(self, patterns_and_callbacks: Dict[str, Callable[[re.Match], None]]):
        ...
        self.logger.info(f"Monitoring directory: {self._log_dir}")
    ...
    def _on_dir_change(self, changed_path):
        ...
        if current_path and changed_path.lower() == current_path.lower():
            ...
            self._last_position = 0 
            self.logger.info(f"Switching to new log: {filename}")
            self._process_new_lines()
    ...
    def _refresh_latest_log(self):
        ...
        try:
            ...
            if latest_file != self._current_log_path:
                ...
                self.logger.info(f"Tracking new log: {os.path.basename(latest_file)}")
